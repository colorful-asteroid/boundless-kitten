// Initial songs put into the library
module.exports = [{
  filename: '50 Cent - Window Shopper.mp3',
  title: 'Window Shopper',
  artist: '50 Cent',
  genre: 'Hip Hop'
}, {
  filename: 'EmSoDep.mp3',
  title: 'Em So Dep',
  artist: 'Antonius Maximus',
  genre: 'Hip Hop'
}, {
  filename: 'Ghost Town DJ\'s- My Boo.mp3',
  title: 'My Boo',
  artist: 'Ghost Town DJ\'s',
  genre: 'R n\' B'
}, {
  filename: 'Gonna Make You Sweat (Everybody Dance Now).mp3',
  title: 'Gonna Make You Sweat',
  artist: 'C + C Music Factory',
  genre: 'Pop'
}, {
  filename: 'LA MAYONESA.wmv.mp3',
  title: 'La Mayonesa',
  artist: 'Chocolate',
  genre: 'Latin Pop'
}, {
  filename: 'Lloyd Banks - I\'m So Fly.mp3',
  title: 'I\'m So Fly',
  artist: 'Lloyd Banks',
  genre: 'Hip Hop'
}, {
  filename: 'Macho Man Randy Savage - Be A Man 2003.mp3',
  title: 'Be A Man (Hulk)',
  artist: '\'Macho Man\' Randy Savage',
  genre: 'Hip Hop'
}, {
  filename: 'Nina Simone  Feeling Good.mp3',
  title: 'Feeling Good',
  artist: 'Nina Simone',
  genre: 'Nina Simone\'s Her Own Category'
}, {
  filename: 'Sean Kingston - Beautiful Girls.mp3',
  title: 'Beautiful Girls',
  artist: 'Sean Kingston',
  genre: 'Pop'
}, {
  filename: 'SHORTY SWING MY WAY - KP & ENVYI.mp3',
  title: 'Shorty Swing My Way',
  artist: 'KP & Envyi',
  genre: 'R n\' B'
}, {
  filename: 'The Game - 100 ft. Drake.mp3',
  title: '100',
  artist: 'The Game ft. Drake',
  genre: 'Hip Hop'
}, {
  filename: 'R. Kelly - Feelin\' On Yo Booty.mp3',
  title: 'Feelin\' On Yo Booty',
  artist: 'R. Kelly',
  genre: 'R n\' B'
}, {
  filename: 'R. Kelly - Ignition (Remix).mp3',
  title: 'Ignition',
  artist: 'R. Kelly',
  genre: 'R n\' B'
}, {
  filename: 'Ginuwine Pony Lyrics.mp3',
  title: 'Pony',
  artist: 'Ginuwine',
  genre: 'R n\' B'
}];

